...
class Language(object):
    ...
    def __init__(self, code):
        ...
        self.program_length = len(code.replace('\r\n','').replace(':',''))

    def generate_code(self, node, call_name=None, call_counter=0):
        code = []
        ...
        for element in node_children:
            if element.getType() in self.STEPS:
                ...
                if call_name != element.text:
                    call_counter = 0

                if call_counter < 40:
                    code.extend(
                        self.generate_code(function, element.text, call_counter+1)
                    )
                else:
                    code.append('!')
                    return code
    ...
    def emit_undefined_error(self, element):
        self.parser.emitErrorMessage(
            "%s %s" % (
                self.parser.getErrorHeader(element),
                "function '%s' undefined" % element.text)
        )
